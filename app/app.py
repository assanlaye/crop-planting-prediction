...
ZONE_TEMP_MAX = {
    "Western":       [32,34,36,37,36,34,31,30,31,32,33,32],
    "North Bank":    [33,35,38,40,39,36,32,31,32,33,34,33],
    "Lower River":   [33,35,38,40,38,35,31,30,32,33,34,33],
    "Central River": [34,36,39,41,39,36,32,31,32,34,35,34],
    "Upper River":   [34,36,40,42,40,37,33,32,33,34,35,34],
}

ZONE_TEMP_MIN = {
    "Western":       [16,17,18,20,22,24,24,23,23,23,20,17],
    "North Bank":    [14,15,17,20,23,24,24,23,23,22,18,15],
    "Lower River":   [15,16,18,21,23,24,24,23,23,22,18,15],
    "Central River": [14,15,17,20,23,24,24,23,23,22,17,14],
    "Upper River":   [13,14,17,20,23,24,24,23,23,21,16,13],
}

ZONE_HUMIDITY = [35,30,30,35,50,65,80,85,82,72,50,38]
ZONE_SOLAR    = [19,21,22,22,21,19,17,17,18,19,20,19]
...
[data-testid="stSidebar"] { background: #1A1F2E !important; border-right: none; }
...
.hero {
    ...
    box-shadow: 0 4px 24px rgba(0,0,0,0.35); border: 1px solid #16A34A;
}
...
.result-suitable {
    ...
    box-shadow: 0 6px 20px rgba(22,101,52,0.4); border: 1px solid #16A34A;
}
...
.result-unsuitable {
    ...
    border-radius: 14px; padding: 1.5rem 1.8rem; margin: 0.8rem 0;
    box-shadow: 0 6px 20px rgba(153,27,27,0.4); border: 1px solid #EF4444;
}
...
.stSelectbox > div > div {
    background: #252B3B !important; color: #E2E8F0 !important;
...
def fetch_weather(zone: str, month: int) -> dict:
    ...
    try:
        ...
        tmax   = [v for v in data.get("temperature_2m_max", [])        if v is not None]
        tmin   = [v for v in data.get("temperature_2m_min", [])        if v is not None]
        rh     = [v for v in data.get("relative_humidity_2m_mean", []) if v is not None]
        solar  = [v or 0.0 for v in data.get("shortwave_radiation_sum", [])]

        total_rain = sum(precip)
        avg_tmax   = sum(tmax)  / len(tmax)  if tmax  else ZONE_TEMP_MAX[zone][month-1]
        avg_tmin   = sum(tmin)  / len(tmin)  if tmin  else ZONE_TEMP_MIN[zone][month-1]
        avg_rh     = sum(rh)    / len(rh)    if rh    else ZONE_HUMIDITY[month-1]
        avg_solar  = sum(solar) / len(solar) if solar else ZONE_SOLAR[month-1]
        ...
        avg_tmax   = ZONE_TEMP_MAX[zone][month - 1]
        avg_tmin   = ZONE_TEMP_MIN[zone][month - 1]
        avg_rh     = ZONE_HUMIDITY[month - 1]
        avg_solar  = ZONE_SOLAR[month - 1]
        rain_3d    = round(total_rain * 3  / 30, 1)
        rain_7d    = round(total_rain * 7  / 30, 1)
        from_api   = False
...
# ══════════════════════════════════════════════════════════════════════════════
# PREDICT  — RF model is the sole decision maker, zero overrides
# ══════════════════════════════════════════════════════════════════════════════
def predict(weather: dict, zone: str):
    """
    Build the 14-feature vector and run it through the Random Forest.
    Returns (prediction: int, probability: float).
    No business-logic overrides — the model decides everything.
    """
...
def render_calendar(zone: str, current_month: int) -> str:
    ...
    for i, score in enumerate(cal):
        ...
        cells += (
            f'<div style="flex:1;height:36px;border-radius:5px;background:{bg};'
            f'color:{fg};display:flex;align-items:center;justify-content:center;'
            f'font-size:10px;font-weight:600;{outline}">'
    ...
    return f'<div style="display:flex;gap:3px;margin-bottom:4px;">{cells}</div>' + legend
...
with tab1:
    ...
    check_btn = st.button("🌱  Can I plant now?", use_container_width=True)

    if check_btn:
        with st.spinner(f"Fetching weather for {zone} — {MONTH_NAMES[month-1]}…"):
        ...
        # ── Result banner — the model has already decided ─────────────────────
        if pred == 1:
            st.markdown(f"""
            ...
            # Display-only hints shown AFTER the result — explain likely reason.
            # These never change the prediction.
            if month == 6 and onset_flag and temp_ok and rain_30d < SEASONAL_THRESH:
                st.info(
                    ...
                    "plant at onset — wait for a follow-up rain event before sowing."
                )
            elif not temp_ok:
                st.warning(
        ...
        # ── Weather tiles ─────────────────────────────────────────────────────
        st.markdown(
            f'<div class="card"><div class="card-title">📡 Weather Data — {src}</div>',
            unsafe_allow_html=True,
        )
        st.markdown(f"""
        <div class="weather-grid">
            ...
            <div class="weather-tile">
                ...
                <span class="wt-val">{rain_3d} mm</span>
        ...
        with col_l:
            ...
            <div class="card">
                ...
                <p style="font-size:0.78rem;color:#64748B;margin:0 0 0.6rem 0;">
                    The AI model learned from these thresholds during training.
                    They explain the prediction but do not override it —
                    the model weighs all 14 features together.
                </p>
                {rows_html}
            </div>""", unsafe_allow_html=True)
        ...
        st.caption(
            f"📡 Source: {src}  ·  Zone: {zone}  ·  {MONTH_NAMES[month-1]}  ·  "
...
with tab2:
    ...
    with col_ctrl:
        ...
        st.caption(
            "Set realistic rainy-season values. "
            "All inputs except the chosen variable stay fixed."
        )
        ...
        s_r7   = st.slider("7-day rainfall (mm)",     0.0, 200.0, 50.0, 1.0, key="s_r7")
        s_tmin = st.slider("Min temperature (°C)",    9.0,  32.0, 23.0, 0.5, key="s_tmin")
        s_tmax = st.slider("Max temperature (°C)",   20.0,  47.0, 33.0, 0.5, key="s_tmax")
        s_hum  = st.slider("Humidity (%)",            5.0,  95.0, 75.0, 1.0, key="s_hum")
        s_sol  = st.slider("Solar radiation (MJ/m²)", 2.0,  29.0, 18.0, 0.5, key="s_sol")
        ...
        run_btn = st.button("▶  Run Analysis", use_container_width=True, key="run_sa")
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    with col_chart:
        if not run_btn:
            ...
            <div style="display:flex;align-items:center;justify-content:center;
                ...
                <div style="text-align:center;color:#475569;">
                    ...
                    <div style="font-size:0.85rem;color:#475569;line-height:1.6;">
                        Set realistic rainy-season inputs on the left,<br>
                        choose a variable to sweep,<br>
                        then click <b style="color:#16A34A">▶ Run Analysis</b>
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
            ...
            # Feature vector — order matches feature_columns exactly:
            ...
            base = [
                s_rain, s_tmin, s_tmax, s_hum,    s_sol,
                s_r3,   s_r7,   s_r30,  onset_s,
                tmean_s, trange_s, sin_s, cos_s, zone_enc,
            ]
            ...
            for val in sweep_vals:
                ...
                # Recompute dependent features when sweeping
                if feat_idx == 5:           # sweeping rain_3d → update onset_flag
                    row[8] = int(val >= ONSET_THRESH)
                if feat_key == "t_mean":    # sweeping mean temp → shift min/max
                    half   = trange_s / 2
            ...
            ax.set_ylabel("P(Suitable for Planting)",
                          fontsize=11, color="#CBD5E1", labelpad=8)
            ax.set_title(
            ...
            for i in range(len(probas) - 1):
                if probas[i] < 0.5 <= probas[i + 1]:
                    ...
                    crossing = sweep_vals[i] + frac * (sweep_vals[i+1] - sweep_vals[i])
                    break
            ...
            if crossing is not None:
                ...
                st.info(
                    ...
                    f"**{sweep_label}** is not the limiting factor here."
                )
            else:
                st.warning(
                    "Suitability stays below 50% — another input is the "
                    "limiting factor. Set the fixed inputs to realistic "
                    "rainy-season values (e.g. 3-day rain ≥ 25mm, "
                    "temp 22–34°C, humidity 75%)."
                )
